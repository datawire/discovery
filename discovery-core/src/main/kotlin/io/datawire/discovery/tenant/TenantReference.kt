...
data class TenantReference(val id: String, val user: String = TenantReference.NONE_USER) {

  /**
   * Constructs a new [TenantReference] from from the provided [JsonObject]. The structure of the JSON is expected to
   ...
   * `
   */
  constructor(json: JsonObject): this(json.getString("id"), json.getString("user", TenantReference.NONE_USER))
   ...
   *
   * The JSON schema is documented below. A [TenantReference] is not required to have a user set therefore the "user"
   * field in the JSON will default to the value of [TenantReference.NONE_USER] when this value is not specified on the
   * source object.
   *
   * `
   * {
