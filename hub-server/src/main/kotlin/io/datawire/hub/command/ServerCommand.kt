...
package io.datawire.hub.command
...
class ServerCommand(application: Application<HubConfiguration>):
    ContextCommand<HubConfiguration>("server", "Run the Hub server", application) {

  private val log = LoggerFactory.getLogger(ServerCommand::class.java)

  override fun run(configuration: HubConfiguration?, context: Context?, namespace: Namespace?) {
